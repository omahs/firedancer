...
fd_pack_tile( fd_cnc_t *              cnc,
  ...
  do {
    ...
    if( FD_UNLIKELY( (!poh_slot) | (!poh_reset_slot) ) ) FD_LOG_ERR(( "poh_slot and/or poh_reset_slot not configured" ));
    ctx->_poh_slot       = poh_slot;
    ctx->_poh_reset_slot = poh_reset_slot;
    ctx->poh_slot          = 0UL;
